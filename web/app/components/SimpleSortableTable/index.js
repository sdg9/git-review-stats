 ...
 */
class SimpleSortableTable extends React.Component {
  ...
  makeTableSortRules() {
    const sortRules = [];
    Object.keys(this.props.columnMetadata).forEach((colName) => {
  ...
  renderTableHead() {
    const thElements = [];
    Object.keys(this.props.columnMetadata).forEach((columnName) => {
  ...
  renderRow(index, rowData) {
    const rowCells = Object.keys(this.props.columnMetadata).map((columnName) => {
